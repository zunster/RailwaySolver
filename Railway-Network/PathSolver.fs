...
module PathSolver =
    type PathSolver(game : Game, upPaths : Dictionary<Label,int> list, downPaths : Dictionary<Label,int> list) =
        ...
        let setPoints idx len (status : PointStatus) =
            [ for i in 0 .. len - 1 -> if i.Equals idx then status else PointStatus.Minus] //change this
